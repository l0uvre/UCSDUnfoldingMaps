    ...
    for (Marker marker : quakeMarkers) {
      if (marker.isInside(map, mouseX, mouseY) && !marker.isHidden()) {
        ...
        for (Marker cityM : cityMarkers) {
          if (cityM.getDistanceTo(lastClicked.getLocation()) > ((EarthquakeMarker) lastClicked).threatCircle()) {
      ...
      for (Marker marker : cityMarkers) {
        if (!marker.isHidden() && marker.isInside(map, mouseX, mouseY)) {
          ...
          for (Marker eq : quakeMarkers) {
            if (lastClicked.getDistanceTo(eq.getLocation()) > ((EarthquakeMarker) eq).threatCircle()) {
              eq.setHidden(true);
            }
          }
